...
static Instruction* getNext(BasicBlock::InstListType& instList, Instruction* currentInst)
{
   for(BasicBlock::iterator I = instList.begin(), E = instList.end();I!=E;++I)
   {
      Instruction* tmp = &*I;
      if(tmp == currentInst)
      {
...
static Value* fix_add(Instruction *InsPoint)
{
   ...
   while(true)
   {
      isLoad = getNext(BB->getInstList(),isLoad);
      if(isa<LoadInst>(isLoad)){
         Instruction* tmp = getNext(BB->getInstList(),isLoad);
   ...
   while(true)
   {
      isCall = getNext(BB->getInstList(),isCall);
   ...
   AllocaInst* a;
   StoreInst* s;
   a = new AllocaInst(I32Ty,NULL,"",BB->getTerminator());
...
static Value* float_add(Instruction* InsPoint)
{
   ...
   //Cast i32 to fp
   isLoad = CastInst::Create(CastInst::SIToFP,isLoad,FTy,"",getNext(BB->getInstList(),isLoad));
...
static Value* alloca_op(Instruction* InsPoint){
   ...
   BasicBlock* BB = InsPoint->getParent();
   for(int i = 0;i < REPEAT;++i){
      a = new AllocaInst(I32Ty,NULL,"",InsPoint);
...
static Value* getelementptr_op(Instruction* InsPoint){
   ...
   for(int i = 0;i < REPEAT;++i){
      ...
      Lhs = builder.CreateGEP(var, Arg);
      //Lhs = GetElementPtrInst::Create(I32Ty, var, Arg, "", InsPoint);
